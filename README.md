...
<div id="gameContainer">
  <div id="ui">
    <span><label>MARIO</label><b id="scoreDisplay">000000</b></span>
    <span><label>🪙</label><b id="coinDisplay">×00</b></span>
    <span><label>WORLD</label><b id="worldDisplay">1-1</b></span>
  ...
  <div id="gameOverScreen" class="screen" style="position:absolute;top:0;left:0;width:100%;height:100%;display:none;flex-direction:column;justify-content:center;align-items:center;z-index:20;">
    ...
    <button class="screen-btn" onclick="startGame()">↺ REINICIAR</button>
  </div>
  ...
  <div id="winScreen" class="screen" style="position:absolute;top:0;left:0;width:100%;height:100%;display:none;flex-direction:column;justify-content:center;align-items:center;z-index:20;">
    ...
    <button class="screen-btn" onclick="nextLevel()">▶ SIGUIENTE NIVEL</button>
    <button class="screen-btn" onclick="startGame()">↺ REINICIAR</button>
  </div>
...
function playTone(freq, type, dur, vol=0.3, startFreq=null) {
  try {
    ...
    osc.type = type;
    if (startFreq) { osc.frequency.setValueAtTime(startFreq, a.currentTime); osc.frequency.linearRampToValueAtTime(freq, a.currentTime + dur); }
    else osc.frequency.setValueAtTime(freq, a.currentTime);
...
const SFX = {
  jump: () => { playTone(600, 'square', 0.12, 0.25, 300); },
  coin: () => { playTone(1200, 'square', 0.08, 0.2); setTimeout(() => playTone(1600, 'square', 0.08, 0.2), 80); },
  block: () => { playTone(220, 'square', 0.1, 0.3); },
  stomp: () => { playTone(200, 'square', 0.15, 0.35, 400); },
  powerup: () => { [523,659,784,1047].forEach((f,i) => setTimeout(() => playTone(f,'square',0.12,0.25), i*80)); },
  hurt: () => { playTone(150,'sawtooth',0.4,0.35,500); },
  die: () => { [600,400,300,200,100].forEach((f,i) => setTimeout(() => playTone(f,'square',0.1,0.3), i*100)); },
  win: () => { [523,659,784,659,784,1047].forEach((f,i) => setTimeout(() => playTone(f,'square',0.15,0.3), i*120)); },
  fireball: () => { playTone(800,'sawtooth',0.06,0.2); },
...
window.addEventListener('keydown', e => {
  ...
  if ((e.code === 'Space' || e.code === 'KeyW' || e.code === 'ArrowUp') && gameState === 'playing' && !isPaused) {
  ...
  if (e.code === 'KeyZ' && gameState === 'playing' && player.fireFlower && fireballCooldown <= 0) {
    spawnFireball();
    fireballCooldown = 25;
    SFX.fireball();
  }
...
function hitBlock(plat, p) {
  if (plat.type === 'qblock' && !plat.hit) {
    plat.hit = true;
    plat.coinGiven = true;
    SFX.block();
    let pu = powerups.find(pw => pw.x >= plat.x && pw.x < plat.x + 32 && !pw.active && !pw.collected);
    if (pu) { pu.active = true; pu.x = plat.x; pu.y = plat.y - 32; SFX.powerup(); }
    else { addScore(200); addCoin(plat.x + 16, plat.y); }
    spawnParticles(plat.x + 16, plat.y, '#ff0', 6);
  } else if (plat.type === 'brick') {
    if (player.state !== 'small') {
      SFX.block();
      spawnParticles(plat.x + 16, plat.y + 16, '#c84', 8);
      addScore(50);
...
function updatePlayer() {
  ...
  for (let pipe of pipes) {
    if (!rectOverlap(player, pipe)) continue;
  ...
  powerups.forEach(pu => {
    ...
    if (!pu.rising) {
      if (pu.type === 'mushroom' || pu.type === 'fire' || pu.type === 'star') {
        pu.vx = pu.vx || 1.5;
        pu.vy = (pu.vy || 0) + GRAVITY;
        pu.x += pu.vx; pu.y += pu.vy;
        platforms.forEach(pl => { if (rectOverlap(pu, pl)) { pu.y = pl.y - pu.h; pu.vy = 0; } });
        pipes.forEach(pi => { if (rectOverlap(pu, pi)) { pu.vx *= -1; } });
        if (pu.x < 0 || pu.x > WORLD_W) pu.vx *= -1;
      }
    }
...
function spawnFireball() {
  fireballs.push({ x: player.x + (player.facing > 0 ? player.w : -12), y: player.y + 8, vx: 8 * player.facing, vy: 0, w: 10, h: 10, alive: true, anim: 0 });
}
...
function drawMountains() {
  ...
  for (let mx = 0; mx < WORLD_W; mx += 320) {
    ...
    drawMountain(bx + 160, H - 52, 90, 80);
  }
}
...
function drawGround() {
  platforms.forEach(pl => {
    ...
    if (pl.type === 'ground') {
      ...
      ctx.fillRect(sx+24, pl.y+24, 2, 8);
      ...
      if (pl.hit) {
        ...
        ctx.fillStyle = '#b07000'; ctx.fillRect(sx+2, pl.y+26, 28, 4);
        ctx.fillStyle = '#fff'; ctx.font = 'bold 20px monospace'; ctx.textAlign = 'center';
        ctx.fillText('?', sx+16, pl.y+22);
      }
    } else {
      ctx.fillStyle = '#884400'; ctx.fillRect(sx, pl.y, pl.w, pl.h);
      ctx.fillStyle = '#aa6622'; ctx.fillRect(sx+2, pl.y+2, pl.w-4, 6);
    }
    if (pl.shake > 0) ctx.restore();
  });
}
...
function drawEnemies() {
  enemies.forEach(e => {
    ...
    if (e.type === 'goomba') {
      ...
      ctx.fillStyle = '#000';
      ctx.beginPath(); ctx.arc(sx+9, e.y+11, 2, 0, Math.PI*2); ctx.fill();
      ...
      ctx.fillStyle = '#000';
      ctx.beginPath(); ctx.arc(sx+10, e.y+2, 1.5, 0, Math.PI*2); ctx.fill();
...
function drawFlag() {
  ...
  ctx.moveTo(sx+2, fy); ctx.lineTo(sx+30, fy+12); ctx.lineTo(sx+2, fy+24); ctx.closePath(); ctx.fill();
}
...
function drawParticles() {
  particleList.forEach(p => {
    ...
    ctx.fillRect(p.x - p.size/2 - cameraX, p.y - p.size/2, p.size, p.size);
    ctx.restore();
  });
}
...
function drawFloatingTexts() {
  floatingTexts.forEach(t => {
    ctx.save(); ctx.globalAlpha = t.life / 60;
    ctx.fillStyle = t.color; ctx.font = 'bold 14px monospace'; ctx.textAlign = 'center';
    ctx.fillText(t.text, px(t.x), t.y);
    ctx.restore();
  });
}
...
function updateHUD() {
  ...
  document.getElementById('bestDisplay').textContent = String(bestScore).padStart(6,'0');
  document.getElementById('worldDisplay').textContent = level + '-1';
}
